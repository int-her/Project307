...
function setStationGPS(stationCd) {
	 ...
	 */
	var url = 'http://openapi.seoul.go.kr:8088/DELETED/xml/SearchLocationOfSTNByIDService/0/5/' + stationCd; 
...
function setStationInfo(stationNm) {
	 ...
	 * 	1	STATION_CD	전철역코드
		...
		4	FR_CODE	외부코드
			(외부코드는 지하철에 역 이름과 함께 적혀있는 역번호로, 외국인의 경우 역명보다 역번호로 문의를 하는 경우가 많음)
	 */
	var url = 'http://openapi.seoul.go.kr:8088/DELETED/xml/SearchInfoBySubwayNameService/0/5/' + stationNm; 
	rest.get(url, null, null,
		function(data, xhr) {
			var code = data.getElementsByTagName("CODE")[0].childNodes[0].nodeValue;
			if (code !== "INFO-000") {
